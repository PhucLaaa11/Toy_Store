...
if (isset($_COOKIE['cc_username'])) {
    ...
    if (isset($_GET['id'])) {
        $p_id = $_GET['id'];
        $sqlSelect1 = "SELECT pid FROM cart WHERE username=? AND pid=?";
        $re = $dblink->prepare($sqlSelect1);
        $re->execute(array("$user", "$p_id"));

        if ($re->rowCount() == 0) {
            $query = "INSERT INTO cart(username, pid, pcount, date) VALUE(?,?,1,CURDATE())";
        } else {
            $query = "UPDATE cart SET pcount = pcount + 1 where username=? and pid=?";
        }
    ...
    $sqlSelect = "SELECT * FROM cart c, product p WHERE c.pid = p.pid and username=?";
    ...
    <table class="table">
        ...
        ?>
            <tr>
                ...
                <td><input id="form1" min="0" name="quantity" value="<?= $row['pcount'] ?>" type="number" class="form-control form-control-sm">
                </td>
                <td>
                    <h6 class="mb-0">
                        <span>&#8363;</span>
                ...
                <td><a href="cart.php?del_id=<?= $row['cart_id'] ?>" class="text-muted text-decoration-none">x</td>
            </tr>
        <?php
        }
        ?>
